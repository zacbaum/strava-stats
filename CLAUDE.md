...
- **Strava API access after the premium trial ends**: since June 30 2026, Standard-tier API access (including this repo's `get_data.py` cron) requires a paid Strava subscription; Strava's MCP connector is subscriber-only too. The dashboard itself needs no API — `activities.csv` is committed and complete to date — but the daily cron will start failing once the subscription lapses (disable `.github/workflows/update_data.yml` at that point, or expect noisy failures). Agreed plan, deliberately not built yet: relay new activities through intervals.icu (free Extended-tier Strava partner; athlete needs no subscription) and rewrite `get_data.py` against intervals.icu's free personal API (API-key auth, two new repo secrets); optionally upgrade later to HealthFit → intervals.icu to remove Strava from the pipeline entirely. Free intervals.icu syncs new activities only — fine, history is already in the CSV. New-activity route polylines would need a per-activity FIT/GPX fetch + encode in the cron. If resubscribing instead: migrate the base URL to `https://www.api-v3.strava.com` before June 1 2027.
